...
public class VoiceClientTest : IDisposable
{
    ...
    public void Setup()
    {
        ...
        mockServer = new MockServer(0, "/v1/voice", handlerLambda);
    }
    ...
    {
        _ = new VoiceClient(customerId, apiKey);
    }
    ...
    public void TestVoiceClientCall()
    {
        ...
        Assert.That(requests.Last().RawUrl, Is.EqualTo("/v1/voice"), "path is not as expected");
        Assert.That(requestBodies.Last(), Is.EqualTo("phone_number=15555555555&message=TestVoiceClientCall&message_type=ARN"));
    ...
    public void TestVoiceClientStatus()
    {
        ...
        mockServer.AddRequestHandler(new MockHttpHandler($"/v1/voice/{refid}", "GET", handlerLambda));

        client.Status(refid);

        Assert.That(requests.Last().HttpMethod, Is.EqualTo("GET"), "method is not as expected");
        Assert.That(requests.Last().RawUrl, Is.EqualTo($"/v1/voice/{refid}?reference_id={refid}"), "path is not as expected");
